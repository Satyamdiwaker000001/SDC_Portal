...
import { 
  Cpu, Trash2, Info, CheckCircle2, Search, UserPlus, Zap, UploadCloud, FileJson, X
} from 'lucide-react';
...
export const CreateTeam = () => {
  ...
  const isManualDisabled = !teamName || !projectId || manualMembers.some(m => !m.userId);
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center border-b border-white/5 pb-4 gap-4">
        ...
        <div className="flex bg-black p-1 border border-white/10 rounded-sm shadow-2xl">
          <button 
            ...
            className={`px-6 py-2 text-[9px] font-black uppercase tracking-widest transition-all ${activeTab === 'manual' ? 'bg-white text-black shadow-[0_0_15px_rgba(255,255,255,0.1)]' : 'text-zinc-500 hover:text-white'}`}
          >
            MANUAL_ASSEMBLY
          </button>
          <button 
            onClick={() => setActiveTab('bulk')} 
            className={`px-6 py-2 text-[9px] font-black uppercase tracking-widest transition-all ${activeTab === 'bulk' ? 'bg-white text-black shadow-[0_0_15px_rgba(255,255,255,0.1)]' : 'text-zinc-500 hover:text-white'}`}
          >
            BULK_INJECTION
          </button>
        </div>
          ...
          >
            ...
            <div className="bg-zinc-900/50 border border-white/5 p-4 relative overflow-hidden">
              ...
              <div className="flex items-start gap-3">
                ...
                <p className="text-[9px] text-zinc-500 font-medium leading-relaxed uppercase tracking-tight">
                  Recommendation: Maintain squad size of 2-4 members. Ensure all identifiers are cross-referenced with the Registry.
                </p>
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-4">
              ...
              <div className="lg:col-span-4 space-y-4 bg-zinc-900/20 p-5 border border-white/5 rounded-sm">
                <div className="space-y-1.5">
                  ...
                  <input 
                    ...
                    className="w-full bg-black border border-white/10 py-3 px-4 text-xs font-bold text-white outline-none focus:border-sky-500/50 transition-all placeholder:text-zinc-800" 
                  />
                </div>
                <div className="space-y-1.5">
              ...
              <div className="lg:col-span-8 space-y-4">
                    ...
                    <AnimatePresence mode="popLayout">
                      {manualMembers.map((member, index) => (
                        <motion.div 
                          ...
                          className="flex gap-3 items-center bg-zinc-950 p-3 border border-white/5 group hover:border-sky-500/20 transition-all"
                        >
                          <div className="w-8 h-8 bg-zinc-900 border border-white/5 flex items-center justify-center text-[10px] font-black text-zinc-700 group-hover:text-sky-400 transition-colors">0{index + 1}</div>
                ...
                {/* MANUAL EXECUTE BUTTON (PROXIMITY) */}
                <button 
          ...
          >
            <div className="bg-zinc-900/50 border border-white/5 p-4 relative overflow-hidden">
              ...
              <p className="text-[9px] text-zinc-500 font-medium uppercase tracking-tight">
                Packet Requirement: CSV must include TEAM_NAME, PROJECT_ID, USER_ID, and ROLE columns for successful injection.
              </p>
            </div>

            <div 
              key="dropzone"
              className="w-full flex flex-col items-center justify-center h-80 border-2 border-dashed border-white/5 bg-zinc-900/10 p-12 space-y-6 group hover:border-sky-500/30 transition-all cursor-pointer relative"
            ...
            <button 
              className="w-full py-5 bg-white text-black font-black text-[11px] uppercase shadow-2xl hover:bg-sky-400 transition-all flex items-center justify-center gap-4 active:scale-[0.98] disabled:opacity-30"
              disabled={!fileName}
            >
              <CheckCircle2 size={18} /> Execute_Deployment_Protocol
            </button>
