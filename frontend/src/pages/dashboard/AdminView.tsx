      ...
      <main className="flex-1 flex flex-col h-screen overflow-hidden bg-[#020203] relative">
        ...
        <header className="h-20 w-full border-b border-white/5 px-10 flex items-center justify-between sticky top-0 bg-black/80 backdrop-blur-xl z-40">
          <div className="flex items-center gap-6">
            <div className="flex items-center gap-4 bg-zinc-950 border border-white/5 px-4 py-2 rounded-sm">
            ...
            <button 
              ...
              className={`flex items-center gap-2 px-4 py-2 border font-black uppercase transition-all rounded-sm ${
                isRecruitmentLive 
          ...
          <div className="flex items-center gap-6 text-right">
            <div>
              <p className="font-black uppercase text-white tracking-widest">{adminName}</p>
              <p className="text-[9px] font-black text-sky-500 uppercase mt-1 italic tracking-widest">Root_Operator</p>
            </div>
            <div className="w-10 h-10 border border-sky-500/40 p-0.5 rotate-45 bg-zinc-900 flex items-center justify-center text-sky-400 font-black">
          ...
          {activeView === 'overview' && (
            <div className="space-y-10 animate-in fade-in duration-500">
              <div className="grid grid-cols-1 md:grid-cols-4 gap-6 w-full">
                {STATS.map((stat, i) => (
                  <div key={i} className="bg-zinc-900/40 border border-white/5 border-l-2 border-l-sky-500 p-6 shadow-xl" style={{ clipPath: 'polygon(0 0, 96% 0, 100% 20%, 100% 100%, 0 100%)' }}>
                    <p className="font-black text-zinc-500 uppercase tracking-widest mb-4">{stat.label}</p>
          ...
          {activeView === 'recruitment' && (
            <div className="space-y-6 animate-in fade-in duration-500">
              <div className="flex justify-between items-center border-b border-white/5 pb-6">
                ...
                <button 
                  onClick={downloadCSV}
                  className="flex items-center gap-2 bg-white text-black px-5 py-2 font-black uppercase hover:bg-sky-500 transition-all rounded-sm"
                >
                  <Download size={14} /> Export_Data.csv
                </button>
              </div>

              <div className="bg-zinc-950/50 border border-white/5 rounded-sm overflow-hidden">
                <table className="w-full text-left">
                  <thead>
                    <tr className="bg-white/5 font-black uppercase text-zinc-500 tracking-widest border-b border-white/10">
                  ...
                  <tbody className="font-bold">
                    {applicants.map((app) => (
                      <tr key={app.id} className="border-b border-white/5 hover:bg-sky-500/5 transition-colors group">
                        ...
                        <td className="p-5 text-white flex items-center gap-3"><User size={12} className="text-sky-500" /> {app.name}</td>
                        <td className="p-5 text-zinc-400 font-mono text-[10px]">{app.email}</td>
                        <td className="p-5 text-zinc-500">{app.class}</td>
                        <td className="p-5 text-right text-emerald-500 uppercase italic">{app.status}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
...
function SidebarLink({ icon: Icon, label, active, onClick }: SidebarProps) {
  return (
    <button onClick={onClick} className={`w-full group relative flex items-center gap-4 px-4 py-3 font-black uppercase tracking-[0.2em] transition-all duration-300 ${active ? 'text-white' : 'text-zinc-500 hover:text-sky-400'}`}>
      {active && <div className="absolute inset-0 bg-sky-500 shadow-[0_0_20px_#0ea5e966] -skew-x-12 z-0" />}
