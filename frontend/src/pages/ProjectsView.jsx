...
// ==========================================
function ProjectFolder({ project, teams, allUsers, onUpdateProject, onDeleteProject, role }) {
  const { user } = useAuth();
  ...
  }, [isOpen, project.id, project.team_id]);
    ...
    <>
        ...
        {isOpen && (
           <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 sm:p-8">
              ...
              <motion.div 
                 ...
                 className="relative w-full max-w-5xl h-[85vh] bg-[#f8f9fa] rounded-3xl shadow-2xl flex flex-col overflow-hidden border border-white/20"
              >
                {/* Header controls */}
                <button 
                   onClick={() => setIsOpen(false)}
                ...
                {/* File tab bars */}
                <div className="flex gap-2 p-3 bg-white border-b border-gray-200 shrink-0 shadow-sm relative z-40 overflow-x-auto">
                  {[
                ...
                <div className="flex-1 overflow-y-auto p-6 md:p-10 custom-scrollbar text-slate-800 relative bg-white">
                      ...
                      {isEditing ? (
                        <form onSubmit={handleSaveProjectDetails} className="space-y-4">
                          ...
                          <input 
                            ...
                            className="w-full bg-white border border-slate-300 rounded-lg px-4 py-3 text-sm text-slate-700 focus:outline-none focus:border-[#00b4d8] shadow-sm"
                          />
                          <textarea 
                            rows="4"
                            value={editData.full_description}
                            onChange={e => setEditData({ ...editData, full_description: e.target.value })}
                            className="w-full bg-white border border-slate-300 rounded-lg px-4 py-3 text-sm text-slate-700 focus:outline-none focus:border-[#00b4d8] shadow-sm resize-none"
                          ...
                          <div className="flex justify-end gap-3 pt-4 border-t border-slate-200">
                            <button 
                              ...
                              className="px-5 py-2 rounded-lg border border-slate-300 text-xs font-bold text-slate-600 hover:bg-slate-50"
                            >Cancel</button>
                            <button 
                              type="submit"
                              className="px-5 py-2 rounded-lg bg-[#00b4d8] text-white text-xs font-bold"
                            >Save Updates</button>
                          </div>
                        </form>
                      ) : (
                        <div className="space-y-6">
                          <div className="flex justify-between items-center border-b border-slate-200 pb-3">
                            <span className="text-[10px] font-mono font-bold text-red-600 border border-red-300 px-3 py-1 bg-red-50 rounded">OFFICIAL PROJECT RECORD</span>
                            <span className="text-xs text-slate-400">STATUS: <strong>{project.status}</strong></span>
                          </div>
                          <div>
                            <h2 className="text-2xl font-black text-slate-900 leading-tight uppercase">{project.name}</h2>
                            ...
                            <p className="text-xs text-slate-600 mt-4 leading-relaxed whitespace-pre-wrap">{project.full_description || 'No detailed documentation specified.'}</p>
                          </div>

                          <div className="grid grid-cols-2 gap-4">
                            {project.live_url ? (
                              <a href={project.live_url} target="_blank" rel="noreferrer" className="flex items-center gap-2 p-3 bg-emerald-50 hover:bg-emerald-100 rounded-xl text-emerald-700 text-xs font-bold uppercase border border-emerald-100">
                                <Globe className="w-4 h-4" /> Hosted Build URL
                              </a>
                            ) : (
                              <div className="flex items-center gap-2 p-3 bg-slate-50 rounded-xl text-slate-400 text-xs font-bold uppercase border border-slate-100">
                                <Globe className="w-4 h-4" /> Undeployed
                              </div>
                            )}

                            {project.github_repo ? (
                              <a href={project.github_repo} target="_blank" rel="noreferrer" className="flex items-center gap-2 p-3 bg-slate-50 hover:bg-slate-100 rounded-xl text-slate-700 text-xs font-bold uppercase border border-slate-200">
                          ...
                          <div className="border-t border-slate-200 pt-5 flex items-center justify-between">
                            <div>
                              <span className="text-[10px] text-slate-400 font-bold uppercase tracking-widest block">Project Progress</span>
                            ...
                            {(role === 'admin' || role === 'developer') && (
                              <div className="flex gap-2">
                                ...
                                <button 
                                  ...
                                  className="px-4 py-2 bg-slate-100 border border-slate-200 text-slate-600 hover:text-slate-900 rounded-lg text-xs font-bold uppercase tracking-wider flex items-center gap-1.5"
                                >
                                ...
                                {role === 'admin' && (
                                  <button 
                                    ...
                                    className="px-4 py-2 bg-rose-50 border border-rose-200 text-rose-500 hover:bg-rose-500 hover:text-white rounded-lg text-xs font-bold uppercase tracking-wider flex items-center gap-1.5"
                                  >
                  ...
                  {activeFile === 'sdlc' && (
                    <div className="max-w-3xl mx-auto space-y-6">
                      <div className="flex items-center justify-between border-b border-slate-200 pb-3 mb-6">
                        <span className="text-xs font-black text-slate-700 uppercase tracking-widest">Predefined SDLC Phases Workflow</span>
                        <span className="text-[10px] text-slate-400 font-bold">SEQUENTIAL PROGRESSION</span>
                      </div>

                      {isLoadingPhases ? (
                        <div className="py-12 text-center text-sm font-bold text-slate-400">Loading SDLC phases...</div>
                      ) : (
                              ...
                              return (
                                <div 
                                  ...
                                  className={`p-3.5 rounded-xl border transition-all text-left flex flex-col justify-between gap-2 relative overflow-hidden ${
                                    phase.is_unlocked 
                                      ? 'cursor-pointer border-slate-200 bg-white hover:border-[#00b4d8]'
                                      : 'bg-slate-50 border-slate-100 opacity-60'
                                  } ${isSelected ? 'border-[#00b4d8] shadow-md shadow-[#00b4d8]/10' : ''}`}
                                >
                                  {isSelected && <div className="absolute top-0 bottom-0 left-0 w-1.5 bg-[#00b4d8]" />}
                                  <div className="flex justify-between items-center">
                                    <span className="text-[9px] font-bold text-slate-400 uppercase tracking-widest">Phase {phase.sequence}</span>
                                    {phase.is_completed ? (
                                  ...
                                  <div>
                                    <h4 className="text-xs font-black text-slate-800 uppercase leading-none">{phase.name}</h4>
                                    <span className="text-[10px] text-slate-400 mt-1 block font-mono">{phase.progress}% Complete</span>
                                  </div>
                                  ...
                                  {!phase.is_unlocked && role === 'admin' && (
                                    <button 
                                      ...
                                      className="mt-2 w-full py-1 bg-slate-100 border border-slate-200 text-slate-600 hover:text-slate-900 rounded-md text-[8px] font-black uppercase tracking-widest"
                                    >
                            ...
                            {selectedPhase ? (
                              <div className="space-y-4">
                                <div className="bg-slate-50 p-4 border border-slate-200 rounded-2xl flex justify-between items-center">
                                  <div>
                                    <h3 className="text-sm font-black text-slate-800 uppercase tracking-wide">Phase {selectedPhase.sequence}: {selectedPhase.name}</h3>
                                    <p className="text-[10px] text-slate-500 mt-0.5 uppercase tracking-widest font-bold">Tasks in this phase contribute to phase completion</p>
                                  </div>
                                  {isTeamLeader && selectedPhase.is_unlocked && (
                                    <button 
                                      onClick={() => setShowAddTaskModal(true)}
                                      className="px-3.5 py-2 bg-[#00b4d8] text-white hover:bg-[#0096c7] rounded-xl text-[9px] font-black uppercase tracking-widest flex items-center gap-1.5 shadow-md shadow-[#00b4d8]/20"
                                    >
                                ...
                                <div className="space-y-3">
                                  {phaseTasks.length === 0 ? (
                                    <div className="py-12 text-center text-slate-400 font-mono text-xs">No tasks registered in this phase.</div>
                                  ) : (
                                    phaseTasks.map(t => {
                                      const assignee = allUsers.find(u => u.id === t.assigned_to);
                                      return (
                                        <div key={t.id} className="p-4 bg-white border border-slate-200 rounded-xl shadow-sm hover:shadow transition-shadow flex justify-between items-center gap-3">
                                          <div>
                                            <h4 className="text-xs font-black text-slate-900 uppercase leading-snug">{t.title}</h4>
                                            <p className="text-[11px] text-slate-500 line-clamp-1">{t.description}</p>
                                            <div className="flex gap-4 pt-1.5 text-[9px] font-bold text-slate-400 uppercase">
                                              <span>Assignee: <strong className="text-slate-600">{assignee ? assignee.name : 'Unassigned'}</strong></span>
                                              {t.due_date && <span>Due: <strong className="text-slate-600">{t.due_date}</strong></span>}
                                            </div>
                                          </div>
                                          <div className="shrink-0">
                                            {t.status === 'COMPLETED' && (
                                              <span className="px-2.5 py-1 bg-emerald-50 border border-emerald-100 rounded-lg text-[8px] font-black uppercase tracking-widest text-emerald-600">Completed</span>
                                            )}
                                            {t.status === 'PENDING_VERIFICATION' && (
                                              <span className="px-2.5 py-1 bg-cyan-50 border border-cyan-100 rounded-lg text-[8px] font-black uppercase tracking-widest text-cyan-600">Pending Review</span>
                                            )}
                                            {t.status === 'IN_PROGRESS' && (
                                              <span className="px-2.5 py-1 bg-amber-50 border border-amber-100 rounded-lg text-[8px] font-black uppercase tracking-widest text-amber-600">In Progress</span>
                                            )}
                                            {t.status === 'PENDING' && (
                                              <span className="px-2.5 py-1 bg-slate-100 border border-slate-200 rounded-lg text-[8px] font-black uppercase tracking-widest text-slate-500">Todo</span>
                                            )}
                                          </div>
                                        </div>
                                      );
                              ...
                              <div className="py-12 text-center text-slate-400 text-xs">Unlock preceding phases to unlock dashboard features.</div>
                            )}
                          </div>
                        </div>
                      )}
                  ...
                  {activeFile === 'documents' && (
                    <div className="max-w-3xl mx-auto space-y-6">
                      <div className="flex items-center justify-between border-b border-slate-200 pb-3 mb-6">
                        <div>
                          <span className="text-xs font-black text-slate-700 uppercase tracking-widest">Mandatory Software Engineering Documentation</span>
                          <p className="text-[9px] text-slate-400 mt-0.5 uppercase font-bold">16 Required Documents + Centralized GitHub Link</p>
                        </div>
                        <span className="text-[10px] text-slate-500 font-bold bg-slate-100 border px-3 py-1 rounded-lg">
                          TL Privilege Uploads
                        </span>
                      </div>

                      {isLoadingDocs ? (
                        <div className="py-12 text-center text-sm font-bold text-slate-400">Syncing repository...</div>
                      ) : (
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                          {documents.map(doc => (
                            <div key={doc.id} className="p-4 bg-slate-50 border border-slate-200 rounded-2xl flex flex-col justify-between gap-4">
                              <div className="flex items-start gap-3">
                                <FileText className={`w-8 h-8 ${doc.file_id ? 'text-[#00b4d8]' : 'text-slate-400'}`} />
                                <div>
                                  <h4 className="text-xs font-black text-slate-800 uppercase leading-none mb-1">{doc.document_type}</h4>
                                  {doc.file_name ? (
                                    <span className="text-[10px] text-slate-500 truncate max-w-[180px] block" title={doc.file_name}>
                                      File: {doc.file_name}
                                    </span>
                                  ) : (
                                    <span className="text-[10px] text-rose-500 font-bold uppercase tracking-widest">Missing Document</span>
                                  )}
                                </div>
                              </div>
                                ...
                                <div className="flex items-center gap-2">
                                  {doc.file_id && doc.file_url && (
                                    <a 
                                      ...
                                      target="_blank"
                                      rel="noreferrer"
                                      className="p-1.5 bg-white border hover:bg-slate-100 rounded-lg text-slate-600 hover:text-slate-900 transition-colors"
                                  ...
                                  {(isTeamLeader || role === 'admin') && (
                                    <label className="p-1.5 bg-white border border-[#00b4d8]/30 hover:bg-[#00b4d8]/10 rounded-lg text-[#00b4d8] cursor-pointer transition-colors">
                    ...
                    <div className="max-w-2xl mx-auto space-y-5">
                      ...
                      <div className="flex flex-col gap-3 border-b border-slate-200 pb-4">
                        <div className="flex items-center justify-between">
                          <span className="text-xs font-black text-slate-700 uppercase tracking-widest">Assigned Personnel</span>
                          <div className="flex items-center gap-2">
                            <span className="text-[10px] text-slate-500 font-bold bg-slate-100 border px-3 py-1 rounded-lg">
                            ...
                            {role === 'admin' && !isChangingTeam && (
                              <button
                                ...
                                className="px-3 py-1 bg-amber-50 border border-amber-200 text-amber-700 hover:bg-amber-100 rounded-lg text-[9px] font-black uppercase tracking-widest flex items-center gap-1"
                              >
                        ...
                        {role === 'admin' && isChangingTeam && (
                          <div className="flex items-center gap-2 bg-amber-50 border border-amber-200 rounded-xl p-3">
                            <select
                              ...
                              className="flex-1 border border-slate-300 rounded-lg px-3 py-2 text-xs text-slate-800 bg-white focus:outline-none focus:border-[#00b4d8]"
                            >
                            ...
                            <button
                              ...
                              className="px-4 py-2 bg-[#00b4d8] text-white rounded-lg text-[9px] font-black uppercase tracking-widest disabled:opacity-50"
                            >
                              {isSubmittingTeam ? 'Saving...' : 'Save'}
                            </button>
                            <button
                              onClick={() => setIsChangingTeam(false)}
                              className="px-3 py-2 bg-slate-100 text-slate-500 rounded-lg text-[9px] font-black uppercase tracking-widest"
                            >
                              Cancel
                            </button>
                          </div>
                      ...
                      {project.team_id ? (
                        isLoadingMembers ? (
                          <div className="py-12 text-center text-sm font-bold text-slate-400">Loading team roster...</div>
                          ...
                          <div className="space-y-3">
                            ...
                            {teamMembers.filter(m => m.designation === 'lead').map(m => (
                              <div key={m.id} className="flex items-center justify-between p-4 bg-cyan-50 border border-cyan-200 rounded-xl shadow-sm">
                                <div className="flex items-center gap-3">
                                  ...
                                  <div>
                                    <h4 className="text-xs font-black text-cyan-900 uppercase leading-none mb-0.5">{m.name}</h4>
                                    <p className="text-[10px] text-cyan-600">{m.email}</p>
                                  </div>
                                </div>
                                <span className="text-[9px] font-black uppercase tracking-wider px-3 py-1 rounded-full bg-[#00b4d8] text-white shadow-sm shrink-0">
                                  👑 Team Leader
                                </span>
                              </div>
                            ))}
                            ...
                            {teamMembers.filter(m => m.designation === 'mentor').map(m => (
                              <div key={m.id} className="flex items-center justify-between p-4 bg-amber-50 border border-amber-200 rounded-xl shadow-sm">
                                <div className="flex items-center gap-3">
                                  ...
                                  <div>
                                    <h4 className="text-xs font-black text-amber-900 uppercase leading-none mb-0.5">{m.name}</h4>
                                    <p className="text-[10px] text-amber-600">{m.email}</p>
                                  </div>
                                </div>
                                <span className="text-[9px] font-black uppercase tracking-wider px-3 py-1 rounded-full bg-amber-500 text-white shadow-sm shrink-0">
                                  ⭐ Mentor
                                </span>
                              </div>
                            ))}
                            ...
                            {teamMembers.filter(m => m.designation !== 'lead' && m.designation !== 'mentor').map(m => (
                              <div key={m.id} className="flex items-center justify-between p-4 bg-slate-50 border border-slate-200 rounded-xl shadow-sm">
                                <div className="flex items-center gap-3">
                                  <div className="w-8 h-8 rounded-full bg-slate-200 flex items-center justify-center text-slate-700 text-xs font-black shrink-0">
                                    {(m.name || '?').charAt(0)}
                                  </div>
                                  <div>
                                    <h4 className="text-xs font-black text-slate-900 uppercase leading-none mb-0.5">{m.name}</h4>
                                    <p className="text-[10px] text-slate-500">{m.email}</p>
                                  </div>
                                </div>
                                <span className="text-[9px] font-black uppercase tracking-wider px-3 py-1 rounded-full bg-slate-700 text-white shadow-sm shrink-0">
                                  {m.designation}
                                </span>
                              </div>
                            ))}
                        ...
                        <div className="py-16 text-center flex flex-col items-center justify-center text-slate-400 bg-slate-50 rounded-2xl border-2 border-dashed border-slate-200">
                          ...
                          <p className="text-sm font-black text-slate-700 mb-1">No Team Assigned</p>
                          {role === 'admin' ? (
                  ...
                  {activeFile === 'feedback' && (
                    <div className="max-w-2xl mx-auto space-y-6">
                      <div className="flex items-center justify-between border-b border-slate-200 pb-3 mb-6">
                        <span className="text-xs font-black text-slate-700 uppercase tracking-widest">Feedback & Discussion Board</span>
                        <span className="text-[10px] text-slate-400 font-bold">PROJECT CHANNEL</span>
                      </div>

                      <div className="space-y-4 max-h-[300px] overflow-y-auto pr-2 custom-scrollbar">
                        {comments.length === 0 ? (
                          <div className="py-8 text-center text-xs text-slate-400 font-mono">No discussions posted yet. start the handshake.</div>
                        ) : (
                          comments.map(c => (
                            <div key={c.id} className="p-3 bg-slate-50 border border-slate-100 rounded-xl space-y-1">
                              ...
                              <p className="text-xs text-slate-700 leading-normal">{c.content}</p>
                            </div>
                          ))
                        )}
                      </div>
                      ...
                      <form onSubmit={handleCreateComment} className="flex gap-2">
                        <input 
                          ...
                          className="flex-1 bg-white border border-slate-300 rounded-xl px-4 py-2.5 text-xs text-slate-950 focus:outline-none focus:border-[#00b4d8] shadow-sm"
                        />
                        <button 
                          type="submit" 
                          disabled={isSubmittingComment}
                          className="px-5 py-2.5 bg-[#00b4d8] hover:bg-[#0096c7] text-white font-black text-[10px] uppercase tracking-wider rounded-xl shadow-md disabled:opacity-50"
                        >
                          Send
                        </button>
                      </form>
      ...
      {showAddTaskModal && selectedPhase && (
        <div className="fixed inset-0 bg-black/85 backdrop-blur-md flex items-center justify-center p-6 z-[120]">
          <motion.div initial={{ scale: 0.95 }} animate={{ scale: 1 }} className="bg-white rounded-3xl p-8 w-full max-w-md space-y-6 shadow-2xl text-slate-800 border">
            <div>
              <span className="text-[9px] uppercase font-bold text-slate-400 tracking-widest">Phase {selectedPhase.sequence} Deployment</span>
              <h3 className="text-base font-black text-slate-900 uppercase mt-1">Assign Task: {selectedPhase.name}</h3>
              <p className="text-[11px] text-slate-500">Assign a work package to team personnel.</p>
            </div>
            
            <form onSubmit={handleCreateTaskSubmit} className="space-y-4">
              <div className="space-y-1">
                <label className="text-[9px] font-bold text-slate-500 uppercase tracking-wider block">Task Title *</label>
                <input 
                  type="text" required
                  placeholder="e.g. Implement routing"
                  value={newTask.title}
                  onChange={e => setNewTask({ ...newTask, title: e.target.value })}
                  className="w-full bg-slate-50 border rounded-xl px-4 py-2.5 text-xs text-slate-950 focus:outline-none focus:border-[#00b4d8]"
                />
              </div>

              <div className="space-y-1">
                <label className="text-[9px] font-bold text-slate-500 uppercase tracking-wider block">Description / Action Items</label>
                <textarea 
                  rows="2"
                  placeholder="Details regarding outputs..."
                  value={newTask.description}
                  onChange={e => setNewTask({ ...newTask, description: e.target.value })}
                  className="w-full bg-slate-50 border rounded-xl px-4 py-2.5 text-xs text-slate-950 focus:outline-none focus:border-[#00b4d8] resize-none"
                />
              </div>

              <div className="grid grid-cols-2 gap-4">
                <div className="space-y-1">
                  <label className="text-[9px] font-bold text-slate-500 uppercase tracking-wider block">Assign Developer *</label>
                  <select 
                    required
                    value={newTask.assigned_to}
                    onChange={e => setNewTask({ ...newTask, assigned_to: e.target.value })}
                    className="w-full bg-slate-50 border rounded-xl px-4 py-2.5 text-xs text-slate-950 focus:outline-none focus:border-[#00b4d8] cursor-pointer"
                  >
                ...
                <div className="space-y-1">
                  <label className="text-[9px] font-bold text-slate-500 uppercase tracking-wider block">Due Date</label>
                  <input 
                    type="date"
              ...
              <div className="flex gap-2 justify-end pt-4">
                <button 
                  ...
                  className="px-4 py-2 border rounded-xl text-[10px] font-black uppercase tracking-wider text-slate-500 hover:bg-slate-50"
                >Cancel</button>
                <button 
                  type="submit" 
                  disabled={isSubmittingTask}
                  className="px-5 py-2 bg-[#00b4d8] hover:bg-[#0096c7] text-white font-black text-[10px] uppercase tracking-wider rounded-xl disabled:opacity-50"
                >
